...
/**
 * Class representing dictionary utils.
 */
class DictionaryUtils {
  ...
  /**
   * Merge the given dictionaries as a one dictionary.
   * @param {...Dictionary} dictionaries
   * @returns {Dictionary}
   */
  static merge(...dictionaries) {
    const words = {};
    const wordsList = [...dictionaries.map(value => value.words)];
    for (let i = 0, len = wordsList.length; i < len; i += 1) {
