...
class Kokoa {
  ...
  static load({ wordFrequencies, wordScores }) {
    ...
    kokoa.wordFrequencies = new Map(Object.entries(wordFrequencies));
    kokoa.wordScores = new Map(Object.entries(wordScores));
    return kokoa;
  }
  ...
  run(document, { hangulOnly = true, threshold = 3 } = {}) {
    ...
    chunks.forEach((chunk) => {
      if (Hangul.isHangulAll(chunk)) {
        ...
        const word = subtexts.map(subtext => [subtext, wordScores.has(subtext) ? wordScores.get(subtext) : 0])
  ...
  save() {
    const { wordFrequencies, wordScores } = this;
    return {
      wordFrequencies: Object.assign({}, ...[...wordFrequencies.entries()].map(([k, v]) => ({ [k]: v }))),
      wordScores: Object.assign({}, ...[...wordScores.entries()].map(([k, v]) => ({ [k]: v }))),
    };
  }
