...
const extractSubtexts = (chunk) => {
  ...
  const subtexts = [];
  const chars = Hangul.flatDisassemble(chunk);
  for (let i = 1, len = chars.length; i <= len; i += 1) {
    const subtext = Hangul.flatAssemble(chars.slice(0, i));
  ...
  update() {
    ...
    wordFrequencies.forEach((frequency, subtext) => {
      if (subtext.length === 1) {
        ...
        const exp = 1 / Hangul.flatDisassemble(subtext).length;
